...
# subprocess.run(['rm', '-rf', 'GoogleAPI_Images'])
subprocess.run([ 'mkdir', 'GoogleAPI_Images'])

# subprocess.run(['rm', '-rf', 'GoogleAPI_Labels'])
subprocess.run([ 'mkdir', 'GoogleAPI_Labels'])
...
for ind in range(0, len(classes)):
    ...
    for line in class_annotations[0:totalNumOfAnnotations]:
        ...
        subprocess.run([ 'aws', 's3', '--no-sign-request', '--only-show-errors', 'cp', 's3://open-images-dataset/'+runMode+'/'+lineParts[0]+".jpg", 'GoogleAPI_Images/'+lineParts[0]+".jpg"])
